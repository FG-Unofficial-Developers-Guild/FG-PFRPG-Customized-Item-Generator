...
local function addEffectsForAbility(nodeItem, sType, sSubType, aAbility)
	...
	if ItemManager.isWeapon(nodeItem) then
		if sSubType == 'melee' then
			aAbilityLookup = MagicItemGeneratorData.aMeleeWeaponAbilities[aAbility.sAbility]
			nCritical = getCritical(nodeItem)
		elseif sSubType == 'ranged' or sSubType == 'firearm' then
			aAbilityLookup = MagicItemGeneratorData.aRangedWeaponAbilities[aAbility.sAbility]
			nCritical = getCritical(nodeItem)
		end
	elseif sType == 'ammunition' then
		aAbilityLookup = MagicItemGeneratorData.aAmmunitionAbilities[aAbility.sAbility]
	elseif ItemManager.isArmor(nodeItem) then
		aAbilityLookup = MagicItemGeneratorData.aArmorAbilities[aAbility.sAbility]
	elseif ItemManager.isShield(nodeItem) then
		aAbilityLookup = MagicItemGeneratorData.aShieldAbilities[aAbility.sAbility]
	end

	local aEffects = {}
	if aAbilityLookup then
		if aAbility.sSubAbility ~= Interface.getString('itemnone') and next(aAbilityLookup.aSubSelection) ~= nil then
			if
				aAbility.sSubSubAbility ~= Interface.getString('itemnone')
...
local function changeDamageBySizeDifference(sDamage, iSizeDifference)
	...
	for _, aDmg in pairs(aDamage) do
		...
		if MagicItemGeneratorData.aAltDamageDice1[sNewDamage] ~= nil then
