...
aMeleeWeaponAbilities = {
    ...
    ["Igniting"] = {
        ...
        aSubSelection = {},
        aEffects = {
            {sEffect = "DMGO: 1d6 fire", nActionOnly = 1, nCritical = 0}
        }
    },
    ["Impact"] = {
        sStringName = "impact",
    ...
    ["Neutralizing"] = {
        ...
        aSubSelection = {},
        aEffects = {
            {sEffect = "IFT: TYPE(earth); 1d6", nActionOnly = 1, nCritical = 0}
        }
    },
    ["Nullifying"] = {
    ...
    ["Quenching"] = {
        ...
        aSubSelection = {},
        aEffects = {
            {sEffect = "IFT: TYPE(fire); 1d6", nActionOnly = 1, nCritical = 0}
        }
    },
    ["Repositioning"] = {
...
aArmorAbilities = {
    ...
    ["Energy resistance"] = {
        ...
        sAura = "faint abjuration",
        aExclusions = {
            "Energy resistance, greater", "Energy resistance, improved"
        },
        sSubSelectionLabel = "Type",
        aSubSelection = {
            ["Acid"] = {
    ...
    ["Spell resistance (13)"] = {
        ...
        sAura = "strong abjuration",
        aExclusions = {
            "Spell resistance (15)", "Spell resistance (17)",
            "Spell resistance (19)"
        },
        sSubSelectionLabel = "",
        aSubSelection = {},
    ...
    ["Spell resistance (15)"] = {
        ...
        sAura = "strong abjuration",
        aExclusions = {
            "Spell resistance (13)", "Spell resistance (17)",
            "Spell resistance (19)"
        },
        sSubSelectionLabel = "",
        aSubSelection = {},
    ...
    ["Spell resistance (17)"] = {
        ...
        sAura = "strong abjuration",
        aExclusions = {
            "Spell resistance (13)", "Spell resistance (15)",
            "Spell resistance (19)"
        },
        sSubSelectionLabel = "",
        aSubSelection = {},
...
local function getDamageBySize(sDamage, sOriginalSize, sItemSize)
    local iSizeDifference = aItemSize[sItemSize:lower()].iPosition -
                                aItemSize[sOriginalSize:lower()].iPosition;
...
function generateMagicItem(nodeItem)
    ...
    local bBonus, bMaterial, nErrorCode, aConflicts =
        checkComboboxes(sType, sSubType, sBonus, sSpecialMaterial, aAbilities);
    if nErrorCode == 1 then
        Comm.addChatMessage({
            text = string.format(Interface.getString("magic_item_gen_error_4"),
                                 aConflicts.sAbility1),
            secret = true,
            icon = "ct_faction_foe"
        });
        return false;
    elseif nErrorCode == 2 then
        Comm.addChatMessage({
            text = string.format(Interface.getString("magic_item_gen_error_5"),
                                 aConflicts.sAbility1, aConflicts.sAbility2),
            secret = true,
            icon = "ct_faction_foe"
        });
        return false;
    elseif nErrorCode == 3 then
        Comm.addChatMessage({
            text = string.format(Interface.getString("magic_item_gen_error_6"),
                                 aConflicts.sAbility1),
            secret = true,
            icon = "ct_faction_foe"
        });
    ...
    local sItemName, iItemCost, iItemWeight, sFullSubType, sItemProperties,
          iArmorPenalty, iArmorMaxDex, iArmorSpellFailure, iSpeed30, iSpeed20,
          iRange, sDamageType, sDamage, sOriginalSize = getItemData(nodeItem);
    local sNewDamage = "";
    ...
    local aAbilityBonus, aAbilityCostBonus, aAbilityExtraCost, aAbilityName,
          aCL, aAura, aEffectList = {}, {}, {}, {}, {}, {}, {};
    for _, aAbility in ipairs(aAbilities) do
        local iAbilityBonus, iAbilityCostBonus, iAbilityExtraCost, sAbilityName,
              iCL, sAura = getAbilityBonusAndCost(aAbility.sAbility, sType,
                                                  sSubType);
        ...
        sDamageType, iRange = getSpecialAbilityData(aAbility.sAbility,
                                                    sDamageType, iRange);
    ...
    local iMaterialCost, iNewWeight, iNewArmorPenalty, iNewArmorMaxDex,
          iNewArmorSpellFailure, iNewSpeed30, iNewSpeed20, bMasterworkMaterial,
          bFragileMaterial, sItemProperties, sDamageType, sAddDescription =
        getMaterialData(sSpecialMaterial, iEnchancementBonus, sType, aSubType,
    ...
    if (not bMasterworkMaterial) or
        (not sEnhancementBonus == Interface.getString("itemnone")) then
        iMasterworkCost = getMasterworkPrice(sType, sItemProperties);
    end
    if bMasterworkMaterial or sEnhancementBonus ~=
    ...
    local iTotalCost = iMaterialCost + iMasterworkCost + iEnhancementCost +
                           iExtraCost;

    local sItemNewName = getItemNewName(sItemName, sEnhancementBonus,
                                        iEnchancementBonus, sSpecialMaterial,
                                        aAbilities, bMasterworkMaterial,
                                        sItemSize)
    ...
    if iEnchancementBonus > 0 then
        ...
        if DataCommon.isPFRPG() and (sType == "weapon" or sType == "ammunition") then
            sDamageType = getDamageTypeByEnhancementBonus(sDamageType,
                                                          iEnchancementBonus);
        end
    end
    ...
    if sAddDescription ~= "" then
        sItemDescription = sItemDescription .. "<h>" .. sSpecialMaterial ..
                               "</h>" .. sAddDescription;
    end
    ...
    populateItemField(nodeItem, 'name', 'string',
                      StringManager.capitalize(sItemNewName));
    populateItemField(nodeItem, 'nonid_name', 'string',
                      StringManager.capitalize(sNewNonIdentifiedName));
    ...
    if sType == "armor" or sType == "shield" then
        ...
        populateItemField(nodeItem, 'spellfailure', 'number',
                          iNewArmorSpellFailure);
    end
    for _, aAbility in pairs(aAbilities) do
        addEffectsForAbility(nodeItem, sType, sSubType, aAbility.sAbility,
                             aAbility.sSubAbility, aAbility.sSubSubAbility);
    end
    ...
    Comm.addChatMessage({
        text = "Generated " .. sItemNewName,
        secret = true,
        icon = "ct_faction_friend"
    }); -- ]]
...
function getAbilities(nodeItem, sType, sSubType)
    ...
    for _, nodeAbility in pairs(DB.getChildren(nodeItem, "abilitieslist")) do
        ...
        aAbility.sSubAbility = DB.getValue(nodeAbility,
                                           "combobox_ability_sub_select");
        aAbility.sSubSubAbility = DB.getValue(nodeAbility,
                                              "combobox_ability_sub_sub_select");
...
function cleanAbility(aAbility, sType, sSubType)
    ...
    if next(aAbilityList[aAbility.sAbility].aSubSelection) == nil then
        ...
        if aAbility.sSubAbility ~= Interface.getString("itemnone") and
            next(
                aAbilityList[aAbility.sAbility].aSubSelection[aAbility.sSubAbility]
                    .aSubSubSelection) == nil then
            aNewAbility.sSubSubAbility = Interface.getString("itemnone");
        else
...
function checkComboboxes(sType, sSubType, sBonus, sMaterial, aAbilities)
    ...
    for key1, aAbility1 in ipairs(aAbilities) do
        for key2, aAbility2 in ipairs(aAbilities) do
            if (key1 ~= key2) then
                local nError = checkForAbilitySelectionError(sType, sSubType,
                                                             aAbility1,
                                                             aAbility2);
                if nError > 0 then
        ...
        local aAbilityList = getAbilityList(sType, sSubType);
        if next(aAbilityList[aAbility1.sAbility].aSubSelection) ~= nil and
            aAbility1.sSubAbility == Interface.getString("itemnone") then
            aConflicts.sAbility1 = aAbility1.sAbility;
            return bBonus, bMaterial, 3, aConflicts;
        elseif next(aAbilityList[aAbility1.sAbility].aSubSelection) ~= nil and
            next(
                aAbilityList[aAbility1.sAbility].aSubSelection[aAbility1.sSubAbility]
                    .aSubSubSelection) ~= nil and aAbility1.sSubSubAbility ==
            Interface.getString("itemnone") then
            aConflicts.sAbility1 = aAbility1.sAbility;
            return bBonus, bMaterial, 3, aConflicts;
        end
...
function contains(aArray, sString)
    for key, value in ipairs(aArray) do
        if value == sString then return true; end
    end
    return false;
end

function getAbilityBonusAndCost(sSpecialAbility, sType, aSubType)
    local iBonus, iBonusCost, iExtraCost, sAbilityName, iCL, sAura = 0, 0, 0,
                                                                     "", 0, "";
...
function getMaterialData(sMaterial, iEnhancingBonus, sType, aSubType,
                         sFullSubType, iWeight, iArmorPenalty, iArmorMaxDex,
                         iArmorSpellFailure, iSpeed30, iSpeed20, iItemBaseCost,
                         sProperties, sDamageType)
    ...
    if aSpecialMaterials[sMaterial] and
        aSpecialMaterials[sMaterial].sAddDescription then
    ...
    elseif sMaterial == Interface.getString("fire_forged_steel") or sMaterial ==
        Interface.getString("frost_forged_steel") then
        if aSubType == "light" then
            iMaterialCost = iMaterialCost + 1000;
        elseif aSubType == "medium" then
    ...
    if aSpecialMaterials[sMaterial] and
        aSpecialMaterials[sMaterial].bAlwaysMasterwork and sType == "armor" and
        iNewArmorPenalty == iArmorPenalty then
        iNewArmorPenalty = iArmorPenalty + 1;
    end
    if iNewArmorPenalty > 0 then iNewArmorPenalty = 0; end
    ...
    if aSpecialMaterials[sMaterial] and
        aSpecialMaterials[sMaterial].bAlwaysMasterwork then
    ...
    return iMaterialCost, iNewWeight, iNewArmorPenalty, iNewArmorMaxDex,
           iNewArmorSpellFailure, iNewSpeed30, iNewSpeed20, bAlwaysMasterwork,
...
function getItemData(databasenode)
    ...
    local sItemName, iItemCost, iItemWeight, sFullSubType, sItemProperties,
          iArmorPenalty, iArmorMaxDex, iArmorSpellFailure, iSpeed30, iSpeed20,
          sItemCost, sDamageType, iRange, sDamage, sOriginalSize = "", 0, 0, "",
                                                                   "", 0, 0, 0,
                                                                   0, 0, "", "",
                                                                   0, "", "";
    ...
    return sItemName, iItemCost, iItemWeight, sFullSubType, sItemProperties,
           iArmorPenalty, iArmorMaxDex, iArmorSpellFailure, iSpeed30, iSpeed20,
           iRange, sDamageType, sDamage, sOriginalSize;
end

function getEnchancementCost(iEnchancementBonus, sType)
    local aBonusPriceArmor = {
        0, 1000, 4000, 9000, 16000, 25000, 36000, 49000, 64000, 81000, 100000
    }
    local aBonusPriceWeapon = {
        0, 2000, 8000, 18000, 32000, 50000, 72000, 98000, 128000, 162000, 200000
    }
    local aBonusPriceAmmunition = {
...
function getItemNewName(sItemName, sEnhancementBonus, iEnchancementBonus,
                        sSpecialMaterial, aAbilities, bMasterworkMaterial,
                        sItemSize)
    local sItemNewName = "";
    if sItemSize:lower() ~= Interface.getString("item_size_medium"):lower() then
        sItemNewName = sItemNewName .. sItemSize .. " ";
    end
    if sEnhancementBonus == Interface.getString("bonus_mwk") and
        (not bMasterworkMaterial) then
    ...
    if sSpecialMaterial ~= Interface.getString("itemnone") then
        sItemNewName = sItemNewName ..
                           aSpecialMaterials[sSpecialMaterial].sStringName ..
                           " ";
    end

    for _, aAbility in pairs(aAbilities) do
        sItemNewName = sItemNewName ..
                           figureAbilityName(aAbility.sAbility,
                                             aAbility.sSubAbility,
                                             aAbility.sSubSubAbility);
    end
...
function changeDamageBySizeDifference(sDamage, iSizeDifference)
    ...
    for _, aDmg in pairs(aDamage) do
        ...
        for iVar = 1, math.abs(iSizeDifference), 1 do
            ...
            sNewDamage = aPositionDamage[aDamageDice[sNewDamage].iPosition +
                             iChange].sDamage;
        end
        if bIsAlt1 then
            if aPositionDamage[aDamageDice[sNewDamage].iPosition].sAltDamage1 ~=
                nil then
                sNewDamage = aPositionDamage[aDamageDice[sNewDamage].iPosition]
                                 .sAltDamage1;
            end
        ...
        if bIsAlt2 then
            if aPositionDamage[aDamageDice[sNewDamage].iPosition].sAltDamage2 ~=
                nil then
                sNewDamage = aPositionDamage[aDamageDice[sNewDamage].iPosition]
                                 .sAltDamage2;
            end
...
local function usingAE()
    return StringManager.contains(Extension.getExtensions(),
                                  "FG-PFRPG-Advanced-Effects");
end

function addEffectsForAbility(nodeItem, sType, sSubType, sAbility, sSubAbility,
                              sSubSubAbility)
    if not nodeItem then return; end
    ...
    local aEffects = {}
    if aAbility then
        if sSubAbility ~= Interface.getString("itemnone") and
            next(aAbility.aSubSelection) ~= nil then
            if sSubSubAbility ~= Interface.getString("itemnone") and
                next(aAbility.aSubSelection[sSubAbility].aSubSubSelection) ~=
                nil then
                aEffects =
                    aAbility.aSubSelection[sSubAbility].aSubSubSelection[sSubSubAbility]
                        .aEffects;
            else
    ...
    if next(aEffects) ~= nil then
        for _, aEffect in ipairs(aEffects) do
            if not aEffect.bAERequired or
                (aEffect.bAERequired and CombatManagerKel) then
                if aEffect.nCritical == 0 or (aEffect.nCritical == nCritical) then
                    ...
                    addEffect(nodeEffectList, sEffect, aEffect.nActionOnly,
                              false);
                end
            end
        end
...
function addAmmoEffect(nodeItem)
    ...
    addEffect(nodeEffectList,
              "IF: CUSTOM(" .. getWeaponTypeName(nodeItem) .. " Attack); ATK: " ..
                  nBonus .. " ranged; DMG: " .. nBonus, 0, false);
end
...
function getWeightBySize(iItemWeight, sOriginalSize, sItemSize)
    ...
    return iItemWeight / aWeightMultiplier[sOriginalSize:lower()].nMultiplier *
               aWeightMultiplier[sItemSize:lower()].nMultiplier;
end
...
function notifyMissingTypeData(sType, sSubType)
    ...
    if not sType or sType == '' then
        ChatManager.SystemMessage(string.format(
                                      Interface.getString(
                                          'magic_item_gen_error_8'), 'type'));
        bNotified = true
    end
    if (not sSubType or sSubType == '') and not sType == 'ammunition' then
        ChatManager.SystemMessage(string.format(
                                      Interface.getString(
                                          'magic_item_gen_error_8'), 'subtype'));
        bNotified = true
    end
    return bNotified
end
